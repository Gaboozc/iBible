...
export function AnalysisTab({ structuralAnalysis, isActive = true }: AnalysisTabProps) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-slate-900 mb-2">{structuralAnalysis.title}</h2>
      ...
      <div className="space-y-4">
        {structuralAnalysis.sections.map((section, idx) => (
          <div key={idx} className="bg-white border border-slate-200 rounded-lg p-6 space-y-3">
            ...
            <p className="text-slate-700 ml-11 leading-relaxed">{section.description}</p>

            <div className="ml-11 bg-amber-50 border border-amber-200 rounded p-3">
              <p className="text-sm">
      ...
      {/* Palabras Repetidas */}
      <section className="bg-gradient-to-br from-purple-50 to-blue-50 border border-purple-200 rounded-lg p-6 space-y-4">
        <div className="flex items-center gap-2">
          <Repeat className="h-5 w-5 text-purple-600" />
          <h3 className="text-lg font-semibold text-slate-900">Palabras y Patrones Repetidos</h3>
        </div>

        <div className="grid md:grid-cols-2 gap-4">
          {structuralAnalysis.repeatedWords.map((item, idx) => (
            <div key={idx} className="bg-white rounded-lg p-4 border border-purple-200 space-y-2">
              ...
              <p className="text-sm text-slate-700 leading-relaxed">{item.significance}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-start gap-3">
        ...
        <div>
          <p className="text-sm text-blue-900 font-medium">Consejo de estudio:</p>
